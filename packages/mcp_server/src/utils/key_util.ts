...
export const generateKeyPair = (outputType: "file" | "console" = "file", bits: number = 2048) => {
  ...
  console.log(" - Public key: ", outputType === "file" ? path.join(outputKeysDir, "public_key.pem") : `\n\n${publicKey}\n\n`);
  console.log(" - Private key: ", outputType === "file" ? path.join(outputKeysDir, "private_key.pem") : `\n\n${privateKey}\n\n`);
};
...
export const encryptWithPublicKey = (
  ...
  const encryptedData = crypto.publicEncrypt(
    {
      key: option?.publicKey || fs.readFileSync(path.join(outputKeysDir, "public_key.pem"), "utf-8"),
...
export const decryptWithPrivateKey = (
  ...
  const decryptedData = crypto.privateDecrypt(
    {
      key: option?.privateKey || fs.readFileSync(path.join(outputKeysDir, "private_key.pem"), "utf-8"),
      padding: option?.padding || crypto.constants.RSA_PKCS1_OAEP_PADDING,
      oaepHash: option?.oaepHash || "sha256",
    },
...
export const createAuthToken = (
  ...
  const signature = encryptWithPublicKey(`${headerBase64}.${payloadBase64}`, { publicKey: option?.publicKey, encoding: "base64url" });
...
export const verifyAuthToken = (
  ...
  try {
    ...
    const decryptedSignature = decryptWithPrivateKey(signature, { privateKey: option?.privateKey, encoding: "base64url" });
    ...
    console.error("[key_util#verifyAuthToken] Error: Token verification failed: ", error);
    throw error;
  }
};
