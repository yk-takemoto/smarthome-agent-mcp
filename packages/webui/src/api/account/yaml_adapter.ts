...
export class YamlAdapter implements AccountAdapter {
  ...
  constructor() {
    this.users = yaml.load(
      fs.readFileSync(path.resolve(`user.${accountEnv}.yaml`), "utf-8"),
    ) as Users;
    this.organizations = yaml.load(
      fs.readFileSync(path.resolve("org.yaml"), "utf-8"),
    ) as Orgs;
  }
  ...
  async getAccountInfo(
    ...
    if (!userInfo) {
      throw new Error(
        "The user information of the authorized user does not exist in the database",
      );
    }
    if (!this.organizations[userInfo.organization]) {
      throw new Error(
        "The organization information of the authorized user does not exist in the database",
      );
    }
    return {
      userId: userId,
