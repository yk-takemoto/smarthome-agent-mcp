...
export class GroqAdapter implements LlmAdapter {
  ...
  constructor(
    protected llmConfig = {
      apiKey:
        JSON.parse(process.env.APP_SECRETS || "{}").GROQ_API_KEY ||
  ...
  async chatCompletions(
    ...
    const chatOtions = {
      ...
      tool_choice:
        options.toolChoice || ("auto" as ChatCompletionToolChoiceOption),
    ...
    try {
      ...
      const chatResponse =
        await this.groqClient.chat.completions.create(chatOtions);
      const choice = chatResponse.choices[0];
      const finishReason = choice.finish_reason;
      // debug
      console.log(
        `[chatCompletions] end -- choices[0].message: ${JSON.stringify(choice.message)} finishReason: ${finishReason}`,
      );
          ...
          finishReason === "tool_calls"
            ? choice.message.tool_calls?.map((tool_call) => {
                return {
                  ...
                  arguments: JSON.parse(tool_call.function.arguments) as Record<
                    string,
                    any
                  >,
                };
