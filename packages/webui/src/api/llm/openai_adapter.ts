...
export class OpenAIAdapter<T extends OpenAI> implements LlmAdapter {
  ...
  constructor(
    protected llmConfig = {
      apiKey:
        JSON.parse(process.env.APP_SECRETS || "{}").OPENAI_API_KEY ||
  ...
  async chatCompletions(
    ...
    const chatOtions = {
      ...
      tool_choice:
        options.toolChoice || ("auto" as OpenAI.ChatCompletionToolChoiceOption),
    ...
    try {
      ...
      const chatResponse =
        await this.openaiClient.chat.completions.create(chatOtions);
  ...
  async textToSpeech(
    ...
    try {
      const response =
        await this.openaiClient.audio.speech.create(speechOtions);
