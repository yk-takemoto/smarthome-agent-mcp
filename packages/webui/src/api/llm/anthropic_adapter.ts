...
export class AnthropicAdapter implements LlmAdapter {
  ...
  constructor(
    protected llmConfig = {
      apiKey:
        JSON.parse(process.env.APP_SECRETS || "{}").ANTHROPIC_API_KEY ||
  ...
  async chatCompletions(
    ...
    try {
      ...
      const chatResponse =
        await this.anthropicClient.messages.create(funcOtions);
      const contents = chatResponse.content;
      const stopReason = chatResponse.stop_reason;
      // debug
      console.log(
        `[chatCompletions] end -- contents: ${JSON.stringify(contents)} stopReason: ${stopReason}`,
      );
