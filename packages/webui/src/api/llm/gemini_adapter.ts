...
export class GeminiAdapter implements LlmAdapter {
  ...
  constructor(
    protected llmConfig = {
      apiKey:
        JSON.parse(process.env.APP_SECRETS || "{}").GEMINI_API_KEY ||
  ...
  async chatCompletions(
    ...
    const modelParams: ModelParams = {
      ...
      toolConfig: {
        functionCallingConfig: {
          mode:
            (String(options.toolChoice).toUpperCase() as FunctionCallingMode) ||
    ...
    try {
      // debug
      console.log(
        "[chatCompletions] start -- updatedMessages: ",
        JSON.stringify(updatedMessages),
      );
      const lastMessage = updatedMessages.pop()?.parts[0].text;
      if (!lastMessage) {
        throw new Error(
          "lastMessage is missing because updatedMessages is empty.",
        );
      }
      ...
      const finishReason =
        chatResponse.candidates && chatResponse.candidates[0].finishReason;
      // debug
      console.log(
        `[chatCompletions] end -- response.text: ${text} response.functionCalls: ${JSON.stringify(funcCalls)} finishReason: ${finishReason}`,
      );
        ...
        resTools = funcCalls
          ? funcCalls?.map((funcCall) => {
              ...
              return {
                id: "",
                name: funcCall.name,
                arguments: JSON.parse(JSON.stringify(funcCall.args)) as Record<
                  string,
                  any
                >,
              };
  ...
  async textToSpeech(
    ...
    try {
      const sorryFormat =
        options.responseFormat === "wav" || options.responseFormat === "aac"
          ? options.responseFormat
          : "mp3";
      const sorry = await fs.readFile(`audio/sorry.ja.${sorryFormat}`);
      const contentType =
        sorryFormat === "mp3" ? "audio/mpeg" : `audio/${sorryFormat}`;
      return {
